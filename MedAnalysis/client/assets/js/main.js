...
function view(event) {
  ...
  if (fileInput.files.length > 0 && selectedOption) {
    ...
    alert(
      "Vui lòng chọn file và một trong các tùy chọn (MRI, CT, Da liễu) trước khi xem!"
    );
  }
}
function predict(event) {
  ...
  if (fileInput.files.length > 0 && selectedOption) {
    ...
    alert(
      "Vui lòng chọn file và một trong các tùy chọn (MRI, CT, Da liễu) trước khi dự đoán!"
    );
  }
}
function home(event) {
...
function readFile(file) {
  const fileType = file.type; // Lấy loại file từ file.type
  var reader = new FileReader();
  if (fileType === "image/png" || fileType === "image/jpeg") {
    niiRange = document.getElementById("niiRange");
    if (!niiRange.classList.contains("hidden")) {
      niiRange.classList.add("hidden");
    }
    // Đọc file ảnh PNG/JPG
    reader.onloadend = function (evt) {
      if (evt.target.readyState === FileReader.DONE) {
        ...
        img.onload = function () {
          ...
          var ctxNav = canvasNav.getContext("2d");

          // Vẽ ảnh lên cả 2 canvas
          ctxMain.drawImage(img, 0, 0);
          ctxNav.drawImage(img, 0, 0);
        };
    ...
    // Xử lý file NIFTI
    if (niiRange.classList.contains("hidden")) {
      niiRange.classList.remove("hidden");
    }
    ...
    alert("Chỉ hỗ trợ các định dạng ảnh PNG, JPG và NIFTI.");
  }
}
...
document.querySelector(".negative").addEventListener("click", () => {
  if (
    ...
    localStorage.getItem("negative") === "false"
  ) {
